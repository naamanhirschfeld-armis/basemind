...
impl ast::AddGraphNodeAttribute {
    fn execute_lazy(&self, exec: &mut ExecutionContext) -> Result<(), ExecutionError> {
        ...
        let stmt = LazyAddGraphNodeAttribute::new(node, attributes, exec.error_context.clone().into());
...
impl ast::AddEdgeAttribute {
    fn execute_lazy(&self, exec: &mut ExecutionContext) -> Result<(), ExecutionError> {
        ...
        let stmt = LazyAddEdgeAttribute::new(source, sink, attributes, exec.error_context.clone().into());
        exec.lazy_graph.push(stmt.into());
        Ok(())
    }
}
        ...
        while i < match_string.len() {
            ...
            for (index, arm) in self.arms.iter().enumerate() {
                ...
                if let Some(captures) = captures {
                    if captures.get(0).expect("missing regex capture").range().is_empty() {
            ...
            for regex_capture in regex_captures.iter() {
                current_regex_captures.push(regex_capture.map(|m| m.as_str()).unwrap_or("").to_string());
            }
            ...
            for statement in &arm.statements {
                ...
                statement
                    ...
                    .with_context(|| format!("matching {} with arm \"{}\"", match_string, arm.regex,).into())
                    .with_context(|| arm_exec.error_context.clone().into())?;
            }

            i += regex_captures.get(0).expect("missing regex capture").range().end;
        }
...
impl ast::ForIn {
    fn execute_lazy(&self, exec: &mut ExecutionContext) -> Result<(), ExecutionError> {
        ...
        for value in values {
            ...
            self.variable.add_lazy(&mut loop_exec, value.into(), false)?;
...
impl ast::Capture {
    fn evaluate_lazy(&self, exec: &mut ExecutionContext) -> Result<LazyValue, ExecutionError> {
        Ok(Value::from_nodes(
            exec.graph,
            exec.mat.nodes_for_capture_index(self.file_capture_index as u32),
            self.quantifier,
        )
        .into())
    }
...
impl ast::Variable {
    fn add_lazy(&self, exec: &mut ExecutionContext, value: LazyValue, mutable: bool) -> Result<(), ExecutionError> {
        match self {
    ...
    fn set_lazy(&self, exec: &mut ExecutionContext, value: LazyValue) -> Result<(), ExecutionError> {
        match self {
...
impl ast::ScopedVariable {
    ...
    fn add_lazy(&self, exec: &mut ExecutionContext, value: LazyValue, mutable: bool) -> Result<(), ExecutionError> {
        if mutable {
            return Err(ExecutionError::CannotDefineMutableScopedVariable(format!("{}", self)));
        }
        let scope = self.scope.evaluate_lazy(exec)?;
    ...
    fn set_lazy(&self, _exec: &mut ExecutionContext, _value: LazyValue) -> Result<(), ExecutionError> {
        Err(ExecutionError::CannotAssignScopedVariable(format!("{}", self)))
    }
}
...
impl ast::UnscopedVariable {
    fn add_lazy(&self, exec: &mut ExecutionContext, value: LazyValue, mutable: bool) -> Result<(), ExecutionError> {
        if exec.config.globals.get(&self.name).is_some() {
            return Err(ExecutionError::DuplicateVariable(format!(" global {}", self)));
        }
    ...
    fn set_lazy(&self, exec: &mut ExecutionContext, value: LazyValue) -> Result<(), ExecutionError> {
        ...
        let value = exec.store.add(value, exec.error_context.clone().into());
        exec.locals.set(self.name.clone(), value.into()).map_err(|_| {
...
impl ast::Attribute {
    fn execute_lazy<F>(&self, exec: &mut ExecutionContext, add_attribute: &mut F) -> Result<(), ExecutionError>
    where
