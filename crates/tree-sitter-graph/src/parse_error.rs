...
impl<'tree> ParseError<'tree> {
    ...
    pub fn display(&'tree self, path: &'tree Path, source: &'tree str) -> impl std::fmt::Display + 'tree {
        ParseErrorDisplay {
            error: self,
            path,
            source,
    ...
    pub fn display_pretty(&'tree self, path: &'tree Path, source: &'tree str) -> impl std::fmt::Display + 'tree {
...
impl TreeWithParseErrorOption {
    ...
    pub fn into_option(self) -> Option<TreeWithParseError> {
        match self.error {
            None => None,
            Some(error) => Some(TreeWithParseError { tree: self.tree, error }),
        }
    }
}
...
fn prefix_style(str: &str) -> impl std::fmt::Display + '_ {
    str
}

fn underline_style(str: &str) -> impl std::fmt::Display + '_ {
    str
}
