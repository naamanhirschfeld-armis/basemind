    ...
    fn set(&mut self, name: Identifier, value: V) -> Result<(), VariableError> {
        match self.values.entry(name) {
            Vacant(v) => self
                ...
                .unwrap_or(Err(VariableError::UndefinedVariable(v.into_key().to_string()))),
            Occupied(mut o) => {
                ...
                if variable.mutable {
                    ...
                    Err(VariableError::CannotAssignImmutableVariable(o.key().to_string()))
                }
            }
        }
    }
