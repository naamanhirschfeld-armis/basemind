...
fn will_skip_divergent_partial_paths() {
    ...
    let symbol_stack_postcondition = create_symbol_stack(&mut graph, &mut partials, (&[("a", None)], None));
...
fn can_apply_offset_to_partial_symbol_stacks() {
    fn verify(stack: NiceSymbolStack, symbol_variable_offset: u32, scope_variable_offset: u32, expected: &str) {
        let mut graph = StackGraph::new();
        let mut partials = PartialPaths::new();
        let stack = create_symbol_stack(&mut graph, &mut partials, stack);
        let with_offset = stack.with_offset(&mut partials, symbol_variable_offset, scope_variable_offset);
...
fn can_unify_partial_symbol_stacks() -> Result<(), PathResolutionError> {
    fn verify(
        ...
        let unified = lhs.unify(&mut partials, rhs, &mut symbol_bindings, &mut scope_bindings)?;
    ...
    fn verify_not(lhs: NiceSymbolStack, rhs: NiceSymbolStack) -> Result<(), PathResolutionError> {
        ...
        let mut scope_bindings = PartialScopeStackBindings::new();
        assert!(
            lhs.unify(&mut partials, rhs, &mut symbol_bindings, &mut scope_bindings)
                .is_err()
        );
        Ok(())
    }
    ...
    verify((&[a, dot, b], None), (&[a, dot, b], None), "a.b", "{}", "{}")?;
    verify((&[a, dot, b], var1), (&[a, dot, b], None), "a.b,%1", "{%1 => <>}", "{}")?;
    verify((&[a, dot, b], None), (&[a, dot, b], var2), "a.b,%2", "{%2 => <>}", "{}")?;
    verify(
        (&[a, dot, b], var1),
        (&[a, dot, b], var2),
        "a.b,%1",
    ...
    verify((&[a_empty], None), (&[a_var1], None), "a/($1)", "{}", "{$1 => ()}")?;
    verify((&[a_var1], None), (&[a_empty], None), "a/($1)", "{}", "{$1 => ()}")?;
...
fn can_create_partial_path_from_node() {
    ...
    let drop_scopes_node = graph.add_drop_scopes_node(NodeID::new_in_file(file, 0)).unwrap();
    ...
    let scope_node = graph.add_scope_node(NodeID::new_in_file(file, 5), false).unwrap();
...
fn can_append_edges() -> Result<(), PathResolutionError> {
    ...
    verify_not(&graph, &[scope0, drop_scopes, scope1], (scope1, jump_to_scope_node));
...
fn can_resolve_to_node() -> Result<(), PathResolutionError> {
    ...
    // resolved node ends up in precondition scope stack
    {
        ...
        let mut ps = PartialPaths::new();
        let mut p = create_partial_path_and_edges(&mut g, &mut ps, &[jump_to_scope_node]).expect("");
    ...
    // resolving succeeds even though the scope stack is finite
    {
        ...
        let mut ps = PartialPaths::new();
        let mut p = create_partial_path_and_edges(&mut g, &mut ps, &[baz_def, jump_to_scope_node]).expect("");
