...
pub fn new() -> StackGraph {
    ...
    *graph.source_info_mut(ref_x) = SourceInfo {
    ...
    graph.node_debug_info_mut(scope_x).add(str_dsl_var, str_arg_scope);
    graph.node_debug_info_mut(scope_x).add(str_dsl_position, str_pos_one);
    graph.node_debug_info_mut(scope).add(str_dsl_var, str_lexical_scope);
    graph.node_debug_info_mut(scope).add(str_dsl_position, str_pos_two);
    graph
