...
pub trait CreateStackGraph {
    ...
    fn pop_scoped_symbol(&mut self, file: Self::File, local_id: u32, symbol: Self::Symbol) -> Self::Node;
...
impl CreateStackGraph for StackGraph {
    ...
    fn definition(&mut self, file: Handle<File>, local_id: u32, symbol: Handle<Symbol>) -> Handle<Node> {
    ...
    fn pop_scoped_symbol(&mut self, file: Handle<File>, local_id: u32, symbol: Handle<Symbol>) -> Handle<Node> {
        self.add_pop_scoped_symbol_node(NodeID::new_in_file(file, local_id), symbol, false)
            .expect("Duplicate node ID")
    }

    fn pop_symbol(&mut self, file: Handle<File>, local_id: u32, symbol: Handle<Symbol>) -> Handle<Node> {
    ...
    fn push_symbol(&mut self, file: Handle<File>, local_id: u32, symbol: Handle<Symbol>) -> Handle<Node> {
        self.add_push_symbol_node(NodeID::new_in_file(file, local_id), symbol, false)
            .expect("Duplicate node ID")
    }

    fn reference(&mut self, file: Handle<File>, local_id: u32, symbol: Handle<Symbol>) -> Handle<Node> {
