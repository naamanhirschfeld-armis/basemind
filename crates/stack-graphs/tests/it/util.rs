...
pub(crate) fn create_scope_node(graph: &mut StackGraph, file: Handle<File>, is_exported: bool) -> Handle<Node> {
    let id = graph.new_node_id(file);
...
pub(crate) fn create_push_symbol_node(
    ...
    graph.add_push_symbol_node(id, symbol, is_reference).unwrap()
}
...
pub(crate) fn create_pop_symbol_node(
    ...
    let id = graph.new_node_id(file);
    let symbol = graph.add_symbol(symbol);
    graph.add_pop_symbol_node(id, symbol, is_definition).unwrap()
}
...
pub(crate) fn create_pop_scoped_symbol_node(
    ...
    let id = graph.new_node_id(file);
    let symbol = graph.add_symbol(symbol);
    graph.add_pop_scoped_symbol_node(id, symbol, is_definition).unwrap()
}
