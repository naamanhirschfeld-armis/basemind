...
fn renaming_path_is_productive() {
    ...
    let p = create_partial_path_and_edges(&mut graph, &mut partials, &[s, foo_def, bar_ref, s]).unwrap();
...
fn identity_path_is_cyclic() {
    ...
    let mut partials = PartialPaths::new();
    let p = create_partial_path_and_edges(&mut graph, &mut partials, &[s, bar_def, bar_ref, s]).unwrap();
...
fn one_step_forward_two_steps_back_path_is_cyclic() {
    ...
    let mut partials = PartialPaths::new();
    let p = create_partial_path_and_edges(&mut graph, &mut partials, &[s, foo_def, foo_ref1, foo_ref2, s]).unwrap();
...
fn two_steps_forward_one_step_back_path_is_cyclic() {
    ...
    let mut partials = PartialPaths::new();
    let p = create_partial_path_and_edges(&mut graph, &mut partials, &[s, foo_def1, foo_def2, foo_ref, s]).unwrap();
...
fn finding_simple_identity_cycle_is_detected() {
    ...
    // test edge cycle detector
    {
        ...
        for edge in &[edge(r, foo_ref, 0), edge(foo_ref, s, 0), edge(s, foo_def, 0)] {
            cd.append(&mut edges, *edge);
            assert!(cd.is_cyclic(&graph, &mut partials, db, &mut edges).unwrap().is_empty());
        }
...
fn stitching_simple_identity_cycle_is_detected() {
    ...
    // test partial path cycle detector
    {
        ...
        assert_eq!(
            enum_set![Cyclicity::StrengthensPostcondition],
            cd.is_cyclic(&graph, &mut partials, &db, &mut paths).unwrap()
        );
    }
}
...
fn stitching_composite_identity_cycle_is_detected() {
    ...
    let mut partials = PartialPaths::new();
    let p0 = create_partial_path_and_edges(&mut graph, &mut partials, &[r, bar_def, foo_ref, r]).unwrap();
    let p1 = create_partial_path_and_edges(&mut graph, &mut partials, &[r, foo_def, bar_ref, r]).unwrap();
