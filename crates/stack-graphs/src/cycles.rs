...
pub struct SimilarPathDetector<P> {
    paths: FxHashMap<PathKey, SmallVec<[P; 4]>>,
    counts: Option<FxHashMap<PathKey, SmallVec<[usize; 4]>>>,
}
...
where
    P: HasPathKey,
{
    fn default() -> SimilarPathDetector<P> {
        SimilarPathDetector::new()
    }
}
    ...
    P: HasPathKey,
{
    ...
    pub fn set_collect_stats(&mut self, collect_stats: bool) {
        if !collect_stats {
            ...
            self.counts = Some(FxHashMap::default());
        }
    }
    ...
    pub fn add_path<Cmp>(&mut self, _graph: &StackGraph, arena: &mut P::Arena, path: &P, cmp: Cmp) -> bool
    where
...
impl<H> AppendingCycleDetector<H> {
    ...
    pub fn append(&mut self, appendables: &mut Appendables<H>, appendage: H) {
        self.appendages
            .push_front(&mut appendables.elements, InternedOrHandle::Database(appendage));
    }
}
    ...
    H: Clone,
{
        ...
        Db: ToAppendable<H, A>,
    {
        ...
        let end_node = match self.appendages.clone().pop_front(&appendables.elements) {
        ...
        loop {
            ...
            loop {
                let appendable = counting_appendages.pop_front(&appendables.elements);
                match appendable {
                    Some(appendage) => {
                        cycle_length += 1;
            ...
            for _ in 0..cycle_length {
                let appendable = remaining_appendages.pop_front(&appendables.elements).expect("").clone();
            ...
            if !prefix_path.edges.is_empty()
                && let Some(cyclicity) = prefix_path.is_cyclic(graph, partials)
            {
                cycles |= cyclicity;
            }
