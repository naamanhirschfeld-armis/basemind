...
/// to the arenas.
fn display_with<'a, D>(mut value: D, graph: &'a StackGraph, partials: &'a mut PartialPaths) -> impl Display + 'a
where
    D: DisplayWithPartialPaths + 'a,
{
    value.prepare(graph, partials);
    DisplayWithPartialPathsWrapper { value, graph, partials }
}
...
/// prepared.
fn display_prepared<'a, D>(value: D, graph: &'a StackGraph, partials: &'a PartialPaths) -> impl Display + 'a
where
    D: DisplayWithPartialPaths + 'a,
{
    DisplayWithPartialPathsWrapper { value, graph, partials }
...
impl PartialScopedSymbol {
    ...
    pub fn equals(&self, partials: &mut PartialPaths, other: &PartialScopedSymbol) -> bool {
        self.symbol == other.symbol
            && equals_option(self.scopes.into_option(), other.scopes.into_option(), |a, b| {
                a.equals(partials, b)
            })
    }
    ...
    pub fn cmp(
        ...
        std::cmp::Ordering::Equal
            ...
            .then_with(|| {
                cmp_option(self.scopes.into_option(), other.scopes.into_option(), |a, b| {
                    a.cmp(partials, b)
                })
            })
    }
...
impl PartialSymbolStack {
    ...
    pub fn push_front(&mut self, partials: &mut PartialPaths, symbol: PartialScopedSymbol) {
        self.length += 1;
        self.symbols.push_front(&mut partials.partial_symbol_stacks, symbol);
    }
    ...
    pub fn push_back(&mut self, partials: &mut PartialPaths, symbol: PartialScopedSymbol) {
        self.length += 1;
        self.symbols.push_back(&mut partials.partial_symbol_stacks, symbol);
    }
    ...
    pub fn pop_front(&mut self, partials: &mut PartialPaths) -> Option<PartialScopedSymbol> {
        let result = self.symbols.pop_front(&mut partials.partial_symbol_stacks).copied();
        if result.is_some() {
            self.length -= 1;
        }
        result
    ...
    pub fn pop_back(&mut self, partials: &mut PartialPaths) -> Option<PartialScopedSymbol> {
        let result = self.symbols.pop_back(&mut partials.partial_symbol_stacks).copied();
        if result.is_some() {
            self.length -= 1;
        }
        result
    ...
    pub fn unify(
        ...
        if !lhs.contains_symbols() && !rhs.contains_symbols() {
            match (lhs.variable.into_option(), rhs.variable.into_option()) {
                ...
                (None, Some(var)) => {
                    symbol_bindings.add(partials, var, PartialSymbolStack::empty(), scope_bindings)?;
    ...
    pub fn equals(mut self, partials: &mut PartialPaths, mut other: PartialSymbolStack) -> bool {
        ...
        equals_option(self.variable.into_option(), other.variable.into_option(), |a, b| a == b)
    }
    ...
    pub fn cmp(
        ...
        cmp_option(self.variable.into_option(), other.variable.into_option(), |a, b| {
            a.cmp(&b)
        })
    }
    ...
    pub fn iter<'a>(&self, partials: &'a mut PartialPaths) -> impl Iterator<Item = PartialScopedSymbol> + 'a {
        self.symbols.iter(&mut partials.partial_symbol_stacks).copied()
    }
    ...
    pub fn iter_unordered<'a>(&self, partials: &'a PartialPaths) -> impl Iterator<Item = PartialScopedSymbol> + 'a {
        self.symbols.iter_unordered(&partials.partial_symbol_stacks).copied()
    }
    ...
    fn ensure_both_directions(&mut self, partials: &mut PartialPaths) {
        self.symbols.ensure_backwards(&mut partials.partial_symbol_stacks);
...
impl PartialScopeStack {
    ...
    pub fn push_front(&mut self, partials: &mut PartialPaths, node: Handle<Node>) {
        self.length += 1;
        self.scopes.push_front(&mut partials.partial_scope_stacks, node);
    }
    ...
    pub fn push_back(&mut self, partials: &mut PartialPaths, node: Handle<Node>) {
        self.length += 1;
        self.scopes.push_back(&mut partials.partial_scope_stacks, node);
    }
    ...
    pub fn pop_front(&mut self, partials: &mut PartialPaths) -> Option<Handle<Node>> {
        let result = self.scopes.pop_front(&mut partials.partial_scope_stacks).copied();
        if result.is_some() {
            self.length -= 1;
        }
        result
    ...
    pub fn pop_back(&mut self, partials: &mut PartialPaths) -> Option<Handle<Node>> {
        let result = self.scopes.pop_back(&mut partials.partial_scope_stacks).copied();
        if result.is_some() {
            self.length -= 1;
        }
        result
    ...
    pub fn equals(self, partials: &mut PartialPaths, other: PartialScopeStack) -> bool {
        self.scopes
            .equals_with(&mut partials.partial_scope_stacks, other.scopes, |a, b| *a == *b)
            && equals_option(self.variable.into_option(), other.variable.into_option(), |a, b| a == b)
    }
    ...
    pub fn cmp(self, partials: &mut PartialPaths, other: PartialScopeStack) -> std::cmp::Ordering {
        std::cmp::Ordering::Equal
            .then_with(|| {
                self.scopes
                    .cmp_with(&mut partials.partial_scope_stacks, other.scopes, |a, b| a.cmp(b))
            })
            .then_with(|| {
                cmp_option(self.variable.into_option(), other.variable.into_option(), |a, b| {
                    a.cmp(&b)
    ...
    pub fn iter_scopes<'a>(&self, partials: &'a mut PartialPaths) -> impl Iterator<Item = Handle<Node>> + 'a {
        self.scopes.iter(&mut partials.partial_scope_stacks).copied()
    }
    ...
    pub fn iter_unordered<'a>(&self, partials: &'a PartialPaths) -> impl Iterator<Item = Handle<Node>> + 'a {
        self.scopes.iter_unordered(&partials.partial_scope_stacks).copied()
    }
    ...
    fn ensure_both_directions(&mut self, partials: &mut PartialPaths) {
        self.scopes.ensure_backwards(&mut partials.partial_scope_stacks);
    ...
    pub fn largest_scope_stack_variable(&self) -> u32 {
        self.variable.into_option().map(ScopeStackVariable::as_u32).unwrap_or(0)
    }
}
...
impl Default for PartialSymbolStackBindings {
    fn default() -> PartialSymbolStackBindings {
        PartialSymbolStackBindings::new()
    }
}
...
impl PartialSymbolStackBindings {
    ...
    pub fn display<'a>(&'a mut self, graph: &'a StackGraph, partials: &'a mut PartialPaths) -> impl Display + 'a {
        display_with(self, graph, partials)
    }
}
    ...
    fn display_with(
        ...
        for (idx, binding) in self.bindings.iter().enumerate() {
            if let Some(binding) = binding.as_ref() {
                ...
                write!(f, "%{} => <{}>", idx + 1, display_prepared(*binding, graph, partials))?;
            }
        }
        write!(f, "}}")
    }
...
impl Default for PartialScopeStackBindings {
    fn default() -> PartialScopeStackBindings {
        PartialScopeStackBindings::new()
    }
}
...
impl PartialPathEdgeList {
    ...
    pub fn push_front(&mut self, partials: &mut PartialPaths, edge: PartialPathEdge) {
        self.length += 1;
        self.edges.push_front(&mut partials.partial_path_edges, edge);
    }
    ...
    pub fn cmp(mut self, partials: &mut PartialPaths, mut other: PartialPathEdgeList) -> std::cmp::Ordering {
        use std::cmp::Ordering;
        while let Some(self_edge) = self.pop_front(partials) {
            if let Some(other_edge) = other.pop_front(partials) {
    ...
    pub fn iter<'a>(&self, partials: &'a mut PartialPaths) -> impl Iterator<Item = PartialPathEdge> + 'a {
    ...
    pub fn iter_unordered<'a>(&self, partials: &'a PartialPaths) -> impl Iterator<Item = PartialPathEdge> + 'a {
        self.edges.iter_unordered(&partials.partial_path_edges).copied()
    }

    fn ensure_both_directions(&mut self, partials: &mut PartialPaths) {
        self.edges.ensure_backwards(&mut partials.partial_path_edges);
...
impl PartialPath {
    ...
    pub fn from_node(graph: &StackGraph, partials: &mut PartialPaths, node: Handle<Node>) -> PartialPath {
        ...
        let mut symbol_stack_postcondition = PartialSymbolStack::from_variable(initial_symbol_stack);
    ...
    pub fn cmp(&self, graph: &StackGraph, partials: &mut PartialPaths, other: &PartialPath) -> std::cmp::Ordering {
        std::cmp::Ordering::Equal
            ...
            .then_with(|| {
                self.symbol_stack_postcondition
                    .cmp(graph, partials, other.symbol_stack_postcondition)
            })
            .then_with(|| {
    ...
    pub fn ensure_both_directions(&mut self, partials: &mut PartialPaths) {
        self.symbol_stack_precondition.ensure_both_directions(partials);
        self.symbol_stack_postcondition.ensure_both_directions(partials);
        self.scope_stack_precondition.ensure_both_directions(partials);
        self.scope_stack_postcondition.ensure_both_directions(partials);
...
impl DisplayWithPartialPaths for &PartialPath {
    fn prepare(&mut self, graph: &StackGraph, partials: &mut PartialPaths) {
        self.symbol_stack_precondition.clone().prepare(graph, partials);
        self.symbol_stack_postcondition.clone().prepare(graph, partials);
        self.scope_stack_precondition.clone().prepare(graph, partials);
        self.scope_stack_postcondition.clone().prepare(graph, partials);
    }
...
impl PartialPath {
    ...
    pub fn ensure_no_overlapping_variables(&mut self, partials: &mut PartialPaths, other: &PartialPath) {
        ...
        self.symbol_stack_precondition =
            self.symbol_stack_precondition
                .with_offset(partials, symbol_variable_offset, scope_variable_offset);
        self.symbol_stack_postcondition =
            self.symbol_stack_postcondition
                .with_offset(partials, symbol_variable_offset, scope_variable_offset);
        self.scope_stack_precondition = self.scope_stack_precondition.with_offset(scope_variable_offset);
        self.scope_stack_postcondition = self.scope_stack_postcondition.with_offset(scope_variable_offset);
    }
    ...
    pub fn resolve_to_node(
        ...
        let mut scope_bindings = PartialScopeStackBindings::new();
        scope_bindings.add(partials, scope_variable, scope_stack).unwrap();
...
impl PartialPath {
    ...
    pub fn concatenate(
        ...
        {
            let unified_symbol_stack = join.unified_symbol_stack.display(graph, partials).to_string();
            let unified_scope_stack = join.unified_scope_stack.display(graph, partials).to_string();
    ...
    fn compute_join(
        ...
        let unified_scope_stack =
            lhs_scope_stack_postcondition.unify(partials, rhs_scope_stack_precondition, &mut scope_bindings)?;
