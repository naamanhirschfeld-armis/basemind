...
impl Handle<Symbol> {
    pub fn display(self, graph: &StackGraph) -> impl Display + '_ {
        DisplaySymbol { wrapped: self, graph }
    }
}
...
impl NodeID {
    pub fn display(self, graph: &StackGraph) -> impl Display + '_ {
        DisplayNodeID { wrapped: self, graph }
    }
}
...
impl Node {
    ...
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayNode { wrapped: self, graph }
    }
}
...
impl DropScopesNode {
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayDropScopesNode { wrapped: self, graph }
    }
}
...
impl PopScopedSymbolNode {
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayPopScopedSymbolNode { wrapped: self, graph }
    }
}
...
impl PopSymbolNode {
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayPopSymbolNode { wrapped: self, graph }
    }
}
...
impl PushScopedSymbolNode {
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayPushScopedSymbolNode { wrapped: self, graph }
    }
}
...
impl PushSymbolNode {
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayPushSymbolNode { wrapped: self, graph }
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        if f.alternate() {
            ...
            write!(
                ...
                "[{} {} {}]",
                self.wrapped.id.display(self.graph),
                if self.wrapped.is_reference { "reference" } else { "push" },
                self.wrapped.symbol.display(self.graph),
            )
        }
    }
...
impl ScopeNode {
    pub fn display<'a>(&'a self, graph: &'a StackGraph) -> impl Display + 'a {
        DisplayScopeNode { wrapped: self, graph }
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        if f.alternate() {
            ...
            write!(
                ...
                "[{}{} scope]",
                self.wrapped.id.display(self.graph),
                if self.wrapped.is_exported { " exported" } else { "" },
            )
        }
    }
}
...
impl StackGraph {
    ...
    pub fn incoming_edge_degree(&self, sink: Handle<Node>) -> Degree {
        self.incoming_edges.get(sink).cloned().unwrap_or(Degree::Zero)
    }
}
...
impl StackGraph {
    ...
    pub fn edge_debug_info(&self, source: Handle<Node>, sink: Handle<Node>) -> Option<&DebugInfo> {
        self.edge_debug_info
            .get(source)
            .and_then(|es| match es.binary_search_by_key(&sink, |e| e.0) {
...
/// Per-source-node debug info for each of its outgoing edges, keyed by the edge's sink.
pub(crate) type EdgeDebugInfos = SmallVec<[(Handle<Node>, DebugInfo); 4]>;
...
pub struct StackGraph {
    ...
    symbol_handles: AHashMap<&'static str, Handle<Symbol>>,
    pub(crate) strings: Arena<InternedString>,
    string_handles: AHashMap<&'static str, Handle<InternedString>>,
    pub(crate) files: Arena<File>,
    file_handles: AHashMap<&'static str, Handle<File>>,
    ...
    pub(crate) edge_debug_info: SupplementalArena<Node, EdgeDebugInfos>,
}
        ...
        for other_file in files.keys().cloned() {
            ...
            for other_node in other.nodes_for_file(other_file) {
                let value: Node = match other[other_node] {
                    ...
                    .into(),
                    Node::Scope(ScopeNode { id, is_exported, .. }) => ScopeNode {
                        id: NodeID::new_in_file(file, id.local_id),
                        _symbol: ControlledOption::default(),
                        _scope: NodeID::default(),
                        is_exported,
...
impl Default for StackGraph {
    fn default() -> StackGraph {
        ...
        StackGraph {
            ...
            symbols: Arena::new(),
            symbol_handles: AHashMap::default(),
            strings: Arena::new(),
            string_handles: AHashMap::default(),
            files: Arena::new(),
            file_handles: AHashMap::default(),
            nodes,
