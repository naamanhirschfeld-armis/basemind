...
impl<T> Drop for Arena<T> {
    fn drop(&mut self) {
        unsafe {
            let items = std::mem::transmute::<&mut [MaybeUninit<T>], &mut [T]>(&mut self.items[1..]) as *mut [T];
            items.drop_in_place();
        }
    }
}
...
impl<T> Arena<T> {
    ...
    pub fn iter_handles(&self) -> impl Iterator<Item = Handle<T>> + use<T> {
    ...
    /// Returns whether this arena holds no instances.  (Index 0 is a reserved sentinel, so an
    /// arena with a single slot is empty.)
    #[inline(always)]
...
impl<H, T> SupplementalArena<H, T> {
    ...
    pub fn get(&self, handle: Handle<H>) -> Option<&T> {
        self.items.get(handle.as_usize()).map(|x| unsafe { &*(x.as_ptr()) })
    }
    ...
    T: Default,
{
    ...
    pub fn get_mut_or_default(&mut self, handle: Handle<H>) -> &mut T {
        ...
        if self.items.len() <= index {
            self.items.resize_with(index + 1, || MaybeUninit::new(T::default()));
        }
        unsafe { std::mem::transmute(&mut self.items[handle.as_usize()]) }
    }
}
...
impl<T> List<T> {
    ...
    pub fn push_front(&mut self, arena: &mut ListArena<T>, head: T) {
        self.cells = arena.add(ListCell { head, tail: self.cells });
    }
...
impl<T> List<T> {
    ...
    pub fn cmp_with<F>(mut self, arena: &ListArena<T>, mut other: List<T>, mut cmp: F) -> std::cmp::Ordering
    where
...
impl<T> ReversibleList<T> {
    ...
    /// Returns an iterator over the elements of this list.
    pub fn iter<'a>(mut self, arena: &'a ReversibleListArena<T>) -> impl Iterator<Item = &'a T> + 'a {
        std::iter::from_fn(move || self.pop_front(arena))
    }
}
...
/// Returned by [`ReversibleList::reverse_reused`][] when the caller asked to reuse a cached
/// reversal that was never computed.  A distinct type (rather than `()`) so callers can match on
/// it and so the error is self-describing.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ReversalNotCached;

impl std::fmt::Display for ReversalNotCached {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("the reversal of this list has not been computed")
    }
}

impl std::error::Error for ReversalNotCached {}
...
impl<T> ReversibleList<T> {
    pub fn equals_with<F>(mut self, arena: &ReversibleListArena<T>, mut other: ReversibleList<T>, mut eq: F) -> bool
    where
        F: FnMut(&T, &T) -> bool,
    {
        loop {
...
where
    T: Ord,
{
    pub fn cmp(self, arena: &ReversibleListArena<T>, other: ReversibleList<T>) -> std::cmp::Ordering {
        self.cmp_with(arena, other, |a, b| a.cmp(b))
    }
}
    ...
    T: Clone,
{
    ...
    pub fn cmp_with<F>(mut self, arena: &mut DequeArena<T>, mut other: Deque<T>, cmp: F) -> std::cmp::Ordering
    where
        F: FnMut(&T, &T) -> std::cmp::Ordering,
    {
