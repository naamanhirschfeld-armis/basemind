...
//! This gives you the opportunity to load additional partial paths into the `Database` before
...
pub trait Appendable {
    ...
    fn display<'a>(&'a self, graph: &'a StackGraph, partials: &'a mut PartialPaths) -> Box<dyn std::fmt::Display + 'a>;
}
...
impl<'a> GraphEdgeCandidates<'a> {
    pub fn new(graph: &'a StackGraph, partials: &'a mut PartialPaths, file: Option<Handle<File>>) -> Self {
        Self {
            graph,
            partials,
            file,
        ...
        R: std::iter::Extend<Edge>,
    {
        result.extend(
            self.graph
                ...
                .filter(|e| self.file.is_none_or(|file| self.graph[e.sink].is_in_file(file))),
        );
    }
...
pub struct Database {
    ...
    symbol_stack_key_cache: FxHashMap<SymbolStackCacheKey, SymbolStackKeyHandle>,
    paths_by_start_node: SupplementalArena<Node, Vec<Handle<PartialPath>>>,
    root_paths_by_precondition_prefix: SupplementalArena<SymbolStackKeyCell, Vec<Handle<PartialPath>>>,
    root_paths_by_precondition_with_variable: SupplementalArena<SymbolStackKeyCell, Vec<Handle<PartialPath>>>,
    root_paths_by_precondition_without_variable: SupplementalArena<SymbolStackKeyCell, Vec<Handle<PartialPath>>>,
...
impl Database {
    ...
    pub fn new() -> Database {
        Database {
            ...
            symbol_stack_key_cache: FxHashMap::default(),
    ...
    pub fn add_partial_path(
        ...
        copious_debugging!(
            ...
            if graph[start_node].is_root() { "root" } else { "node" },
        ...
        if graph[start_node].is_root() {
            ...
            if !key.is_empty() {
                match symbol_stack_precondition.has_variable() {
                    true => self.root_paths_by_precondition_with_variable[key.back_handle()].push(handle),
                    false => self.root_paths_by_precondition_without_variable[key.back_handle()].push(handle),
                }
            }
        ...
        match symbol_stack {
            ...
            None => {
                    ...
                    .chain(self.root_paths_by_precondition_without_variable.iter())
                {
                    ...
                    {
                        for path in paths {
                            copious_debugging!("        Found path {}", self[*path].display(graph, partials));
                        }
                    }
                    result.extend(paths.iter().copied());
                }
    ...
    pub fn iter_partial_paths(&self) -> impl Iterator<Item = Handle<PartialPath>> + use<> {
    ...
    pub fn ensure_both_directions(&mut self, partials: &mut PartialPaths) {
        for path in self.partial_paths.iter_handles() {
            self.partial_paths.get_mut(path).ensure_both_directions(partials);
        }
    }
...
impl<'a> DatabaseCandidates<'a> {
    pub fn new(graph: &'a StackGraph, partials: &'a mut PartialPaths, database: &'a mut Database) -> Self {
        Self {
            graph,
            partials,
            database,
...
impl SymbolStackKey {
    ...
    fn empty() -> SymbolStackKey {
        SymbolStackKey { symbols: List::empty() }
    }
...
impl<H> ForwardPartialPathStitcher<H> {
    ...
    pub fn into_stats(mut self) -> Stats {
        if let (Some(stats), Some(similar_path_detector)) = (&mut self.stats, self.similar_path_detector) {
...
impl<H: Clone> ForwardPartialPathStitcher<H> {
        ...
        C: ForwardCandidates<H, A, Db, Err>,
    {
        ...
        for (new_partial_path, new_cycle_detector) in self.extensions.drain(..) {
            let check_similar_path = new_has_split
                && (!self.check_only_join_nodes
                    || candidates.get_joining_candidate_degree(&new_partial_path) == Degree::Multiple);
            let (graph, partials, _) = candidates.get_graph_partials_and_db();
            if check_similar_path
                && let Some(similar_path_detector) = &mut self.similar_path_detector
                && similar_path_detector.add_path(graph, partials, &new_partial_path, |ps, left, right| {
            ...
            {
                copious_debugging!(" extension {}", new_partial_path.display(graph, partials));
        ...
        if let Some(stats) = &mut self.stats {
            ...
            if extension_count == 0 {
                stats.terminal_path_length.record(partial_path.edges.len());
            }
        }
        candidate_count
    }
        ...
        E: Fn(&StackGraph, &mut PartialPaths, &PartialPath) -> bool,
    {
        ...
        while let Some((partial_path, cycle_detector, has_split)) = self.queue.pop_front() {
            let (graph, partials, _) = candidates.get_graph_partials_and_db();
            copious_debugging!("--> Candidate partial path {}", partial_path.display(graph, partials));
            if self.initial_paths_in_queue > 0 {
                self.initial_paths_in_queue -= 1;
            } else if !extend_while(graph, partials, &partial_path) {
                copious_debugging!("    Do not extend {}", partial_path.display(graph, partials));
                continue;
            }
...
impl ForwardPartialPathStitcher<Edge> {
        ...
        F: FnMut(&StackGraph, &mut PartialPaths, &PartialPath),
    {
        ...
        let initial_paths = graph
            ...
            .collect::<Vec<_>>();
        let mut stitcher = ForwardPartialPathStitcher::from_partial_paths(graph, partials, initial_paths);
...
pub struct Stats {
    ...
    pub terminal_path_length: FrequencyDistribution<usize>,
...
impl std::ops::AddAssign<Self> for Stats {
    fn add_assign(&mut self, rhs: Self) {
        ...
        self.terminal_path_length += rhs.terminal_path_length;
