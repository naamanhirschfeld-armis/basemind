...
| **Code search** | Find source code by meaning or by term — `mode` picks the lane: `semantic` (vector KNN, the default) or `keyword` (native BM25 exact-term) over code chunks derived from the cached parse; returns pointers, fetch bodies with `get_chunk`. Needs `--features code-search`. | `search_code` · `get_chunk` |
