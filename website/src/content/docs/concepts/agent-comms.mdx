...
<Aside type="caution">
Unix only. The comms broker uses Unix domain sockets, so it's not available on Windows. To use
basemind on Windows, build without `--features comms` and skip the comms tools.
</Aside>
