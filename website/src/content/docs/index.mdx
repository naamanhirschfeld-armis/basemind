...
`basemind` program for you on first use.
...
Not using a plugin host? basemind also runs as a plain [MCP server](/start/installation/#2-as-an-mcp-server)
or a [standalone CLI](/reference/cli/). See the full [installation guide](/start/installation/).
