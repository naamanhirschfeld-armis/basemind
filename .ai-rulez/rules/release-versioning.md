...
## Surfaces (all updated by `release:bump`)
...
- **RC** (`X.Y.Z-rc.N`): the npm tag becomes `beta`, the PyPI form becomes `X.Y.ZrcN` (regex
  conversion in `release:bump` and in `publish.yml`'s meta job). Cargo accepts the original
  form.

## Tag → workflow

Tags MUST be `v<version>` (e.g. `v0.1.0`, `v0.1.0-rc.1`). The publish workflow
(`.github/workflows/publish.yml`) is gated on the `v[0-9]+.*` pattern and reads the version
